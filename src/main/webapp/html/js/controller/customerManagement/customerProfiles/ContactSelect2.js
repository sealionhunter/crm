Ext.define('CRM.controller.customerManagement.customerProfiles.ContactSelect2', {
    extend: 'Ext.app.Controller',
    views: [ 'customerManagement.customerProfiles.ContactSelect2' ],
    stores: [ 'customerManagement.customerProfiles.ContactSelect',
            'customerManagement.customerProfiles.ContactCanSelect' ],
    init: function() {
        this.control({
            'contactselect2 > gridpanel[id=contactSListForm]': {
                itemdblclick: this.addContact,
//                selectionchange: this.contactSListFormSelectionchange
            },
//            'contactselect2 > gridpanel[id=contactSelectForm]': {
//                itemdblclick: this.removeContact,
//                selectionchange: this.contactSelectFormSelectionchange
//            },
//            'contactselect2 button[action=contactSelectAdd]': {
//                click: this.addContact
//            },
            'contactselect2 button[action=select]': {
                click: this.removeContact
            },
            'contactselect2 gridpanel[id=contactSListForm] button[id=searchText_contactBtn]': {
    ...
    viewInit: function(flag) {
        var win = Ext.widget('contactselect2');
        var objectID = '';
        var objectFlag = '';
        var objectIDValue = '';
//        if (flag == 1) {
...
//        }
//        box.confirm('删除', '确定移除所选联系人？', function(result) {
//            if (result == 'yes') {
//                if (record) {
//                    Ext.Ajax.request({
