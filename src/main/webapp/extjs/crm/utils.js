...
Ext.TaskMgr.start({
	run : function() {
		...
		clock.update('<font size="2.5">'
				+ Ext.Date.format(new Date(), 'Y-m-d G:i:sA') + ' '
...
CRM.Utils.prototype = {
	...
	trimSpaceSearch : function(obj) {
		...
		if (obj.alias === 'widget.textfield') {
			...
			jsonChange = jsonChange.replace(new RegExp('\\[', "gm"), '\\[')
		...
		if (form.getForm().isValid()) {
			...
			function success(response) {
				...
				if (responseText.CusotmerNameRepeat) {
					box.confirm('提示', responseText.CusotmerNameRepeat,
							function showResult(btn) {
								if (btn === 'no') {
									me.updateRecord(button,
	...
	delRecordsCheck : function(grid, url, idString) {
		...
		var checkRecordIDs = new Array();
		for (var i = 0; i < checkRecord.length; i++) {
			unSortRecordIDs[grid.getStore().indexOf(checkRecord[i])] = checkRecord[i]
					.get(idString);
		}
	...
	query : function(button) {
		if (button.up('grid').down('form').isHidden()
			...
			if (!button.up('grid').down('form').isHidden()) {
				...
				var searchValue = button.up('toolbar').down('#searchText')
						.getValue().toString();
	...
	showContactTrackInfo : function(obj, componentID, contactID) {
		obj.style.color = "purple";
		obj.style.cursor = "pointer";
		var record = Ext.getCmp(componentID).store.findRecord('contactID',
				contactID);
...
window.document.onclick = function() {
	...
	if (typeof (messageList) != 'undefined') {
		...
		if (!((xPos >= messageBtnXY[0])
				&& (xPos <= messageBtnXY[0] + messageBtnWidth)
				&& (yPos >= messageBtnXY[1]) && (yPos <= messageBtnXY[1]
				+ messageBtnHeight))) {
...
Ext.data.Types.DATETIMETYPE = {
	convert : function(v, data) {
		if (v != '' && v != null) {
			return Ext.Date.format(new Date(v.substr(0, 4), v.substr(5, 2) - 1,
					v.substr(8, 2), v.substr(11, 2), v.substr(14, 2)),
					'Y-m-d H:i');
		}
	},
	sortType : st.asDate,
...
Ext
		.define(
				'CRM.store.commonStore',
				{
					...
					/**
					 * @private Called internally when a Proxy has completed a
					 *          load request
					 */
					onProxyLoad : function(operation) {
						var me = this, resultSet = operation.getResultSet(), records = operation
								.getRecords(), error = operation.getError(), successful = operation
								.wasSuccessful();
						...
						// Loading should be set to false before loading the
						// records.
						// loadRecords doesn't expose any hooks or events until
						// refresh
						// and datachanged, so by that time loading should be
						// false
						me.loading = false;
						if (successful) {
						...
						// TODO: deprecate this event, it should always have
						// been 'load' instead. 'load' is now documented, 'read'
						// is not.
						// People are definitely using this so can't deprecate
						// safely until 2.x
						if (me.hasListeners.read) {
							me.fireEvent('read', me, records, successful);
						}

						// this is a callback that would have been passed to the
						// 'read' function and is optional
						Ext.callback(operation.callback, operation.scope || me,
